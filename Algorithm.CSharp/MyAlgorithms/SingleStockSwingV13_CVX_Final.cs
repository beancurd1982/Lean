...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class SingleStockSwingV13_CVX_Final : QCAlgorithm
    {
        // ==========================
        //    CVX 股票最优参数配置（G1 组合）
        // ==========================
        private string _ticker = "CVX"; // 目标股票代码设置为 CVX
        private decimal _initialCash = 5000m;
        private decimal _monthlyContribution = 500m;

        private decimal _buyFraction = 0.30m;
        private int _smaLength = 80; // SMA 周期：80 天
        private decimal _takeProfitUp = 0.10m; // 触发追踪止盈的门槛不变 (10%)
        private decimal _trailingDrop = 0.04m; // 追踪止盈回撤阈值：4%

        // 【V13 新增/优化参数】: 用于控制交易频率
        private decimal _minBuyAmount = 2500m; // 最小买入金额限制
        private decimal _buyThreshold = 0.04m; // 买入阈值：价格必须比 SMA 低 4% (0.04) 才买入

        // 硬止损参数 (3%)
        private decimal _stopLossPct = 0.03m; // 硬性止损：3%
        ...
        public override void Initialize()
        {
            ...
            if (decimal.TryParse(buyThreshParam, NumberStyles.Any, CultureInfo.InvariantCulture, out var bt))
                _buyThreshold = bt;
